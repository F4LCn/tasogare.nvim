...
local Groups = require("tasogare.groups")
...
function M.extras()
  local Extra = require("tasogare.extra")
...
function M.plugins()
  ...
  for _, name in ipairs(names) do
    ...
    t[#t + 1] = {
      link(repo, group.url),
      link(("`%s`"):format(name), "lua/tasogare/groups/" .. name .. ".lua"),
    }
  end
  return M.table(t)
end
...
function M.update()
  local config = Docs.extract("lua/tasogare/config.lua", "\n(--@class tasogare%.Config.-\n})")
